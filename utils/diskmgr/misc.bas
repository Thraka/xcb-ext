...
DECLARE FUNCTION IIF AS BYTE (pEvalVar AS BYTE, pRetValTrue AS BYTE, pRetValFalse AS BYTE) STATIC SHARED
DECLARE FUNCTION IIF AS STRING * 96 (pEvalVar AS BYTE, pRetValTrue AS STRING * 96, pRetValFalse AS STRING * 96) STATIC SHARED OVERLOAD

'CALL pause(0.5,true)
'end

FUNCTION IIF AS BYTE (pEvalVar AS BYTE, pRetValTrue AS BYTE, pRetValFalse AS BYTE) STATIC SHARED
	IF pEvalVar = FALSE THEN RETURN pRetValFalse
	RETURN pRetValTrue
END FUNCTION

FUNCTION IIF AS STRING * 96 (pEvalVar AS BYTE, pRetValTrue AS STRING * 96, pRetValFalse AS STRING * 96) STATIC SHARED OVERLOAD
	IF pEvalVar = FALSE THEN RETURN pRetValFalse
	RETURN pRetValTrue
END FUNCTION
